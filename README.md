...
เนื้อหาผลงานเรียบเรียงมาจาก second-brain vault ซึ่งมีข้อมูลภายในปนอยู่เต็มไปหมด รายละเอียดเต็มอยู่หัวไฟล์ `content/profile.ts` สรุปสั้น ๆ:
...
npm run build           # รันให้อัตโนมัติผ่าน prebuild
```

รันใน CI เป็นด่านแรกด้วย ฉะนั้นถ้าวันหลัง copy ข้อความจาก vault มาวางแล้วลืมล้าง มันจะตกตั้งแต่ตรงนั้น ไม่ขึ้นเว็บ

ถ้าตกแล้วมั่นใจว่าข้อความนั้นปลอดภัยจริง ให้เติมใน `ALLOW` ในสคริปต์พร้อมเหตุผลกำกับ
